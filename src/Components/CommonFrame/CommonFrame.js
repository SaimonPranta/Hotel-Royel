...
const CommonFrame = ({ info }) => {
    ...
    return (
        <div className="sub_common_contaienr ">
                        ...
                        {
                            info.firstName && <p className='room-name'>Room {info.firstName} {info.lastName}</p>
                        }
                        {
                            info.foodName && <p className='room-name'>{info.foodName}</p>
                        }
                        ...
                        {
                            info.bed && <span><img className='similar-icon' src={bed} /> :{info.bed}</span>
                        }
                        {
                            info.person && <span><img className='similar-icon' src={person} /> :{info.sleep}</span>
                        }
                        {
                            info.price && <span className='tk'>Tk {info.price} </span>
                        }
                        {
                            info.firstName && <NavLink to='/protected'><button>Book Now</button></NavLink>
                        }
                    </div>
    );
};
